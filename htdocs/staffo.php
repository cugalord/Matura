...
<body class="body">
  ...
  <?php
    ...
    $stmt = "SELECT StaffID,S_Name,S_Surname,S_Username FROM Staff";

    $sth = ibase_query($dbh, $stmt) or die(ibase_errmsg());
    echo '<table><tr><th>ID</th><th>Ime</th><th>Priimek</th><th>Username</th></tr>';
    while ($row = ibase_fetch_object($sth)) {
        echo '<tr><td>' . $row->STAFFID . '</td><td>' . $row->S_NAME . '</td><td>' . $row->S_SURNAME . '</td><td>' . $row->S_USERNAME . '</td></tr>';
    }
    echo '</table>';
  ?>
</body>
