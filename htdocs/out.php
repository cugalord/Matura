...
<form>
  ...
  if (isset($_GET['id'])) {
    ...
    $stmt = "SELECT * FROM Out_flow('" . $sname . "')";

    $sth = ibase_query($dbh, $stmt) or die(ibase_errmsg());
    echo '<table><tr><th>ID</th><th>Ime</th></tr>';
    $cenaSum = 0;
    while ($row = ibase_fetch_object($sth)) {
        echo '<tr><td>' . $row->ID . '</td><td>' . $row->IME . '</td></tr>';
    }
    echo '</table>';

    echo '<form action="izdaja.php" method="get">';
    echo '<div class="row"><div class="col-100">';
    echo '<button type="submit" name="gumb" value="'.$sname.'">Izdaja</button>';
    echo '</div></div>';
    echo '</form>';
  }
  ?>
